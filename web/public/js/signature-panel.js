...
(function () {
  ...
  function renderPreview(name) {
    ...
    // Ensure the web font is loaded before drawing on canvas
    if (document.fonts && document.fonts.load) {
      document.fonts.load(fontString, name || 'Signature').then(function() {
        if (nameInput.value.trim() !== name) return;
        _drawOnCanvas(name, font, fontSize, color, fontString);
      }).catch(function() {
        _drawOnCanvas(name, font, fontSize, color, fontString);
      });
    } else {
  ...
  function _drawOnCanvas(name, font, fontSize, color, fontString) {
    var padding = 4;
